...
```bash
# add the language code to LANGUAGES in tools/messages.py, then:
uv run python tools/messages.py          # creates or updates django.po, compiles django.mo
# fill the msgstr lines, run it again, and check:
uv run python tools/messages.py --check  # CI fails on stale or untranslated catalogs
```

For an application's own strings, use Django's standard `makemessages`/`compilemessages`
and point `LOCALE_PATHS` at your project's `locale` directory.
