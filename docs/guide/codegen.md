...
python manage.py devx_openapi config.urls.api --against openapi-baseline.json --output openapi-baseline.json
```

`--against` fails on breaking changes to a previous document and, with `--output`, writes
the current one afterwards ([system checks](checks.md#managepy-devx_openapi-against)).
...
- Non-2xx responses raise `ApiError` with status, body and headers. HTML, plain-text and
  malformed JSON error bodies are retained for inspection. Documented non-2xx JSON
  responses are parsed into `Operation.errors` for generators building on
  `ninja_devx.codegen.openapi`. Pydantic clients validate
  successful JSON bodies; TypedDict and TypeScript types do not add runtime validation.
- Multipart requests with simple scalar fields and optional binary files are supported.
  A required file is no longer needed.
- Cookie parameters are sent as a `Cookie` header. Python supports SSE
  (`text/event-stream`) responses as a line iterator (`Iterator[str]`/`AsyncIterator[str]`);
  the TypeScript client rejects streaming explicitly.
- Tagged unions use the OpenAPI `discriminator`: pydantic clients get
  `Annotated[A | B, Field(discriminator=...)]`; TypeScript gets the union.
  Custom multipart encoding, nested/array form fields, other non-JSON bodies, deepObject/custom
  styles, query `explode=False`, and external references
  currently raise a generation error. They are never silently omitted, including optional
  parameters. Configure session cookies on the underlying HTTP client/browser as needed.
...
Inline multipart bodies become named models (a required file is optional now). Python uses
...
export responses; SSE responses are generated by the Python client as a line iterator
(`Iterator[str]`/`AsyncIterator[str]`) and rejected by the TypeScript client rather than
